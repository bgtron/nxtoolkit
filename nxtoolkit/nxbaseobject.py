...
class BaseNXObject(NxSearch):
    """
    This class defines functionality common to all NX objects.
    ...
    @classmethod
    def get_deep_switch_classes(cls, include_concrete=False):
        """
        Get all the switch classes needed for this nxtoolkit class and
        all of its children.
        :return: list of all switch classes
        """
        resp = cls._get_switch_classes()
        for child_class in cls._get_children_classes():
            resp.extend(child_class.get_deep_switch_classes(include_concrete=include_concrete))
        if include_concrete:
    ...
    def get_deep(cls, full_data, working_data, parent=None, limit_to=[], subtree='full', config_only=False):
        ...
        for item in working_data:
            for key in item:
                if key in cls._get_switch_classes():
                    ...
                    if 'children' in item[key]:
                        for child in item[key]['children']:
                            for switch_class in child:
                                class_map = cls._get_toolkit_to_switch_classmap()
                                if switch_class not in class_map:
                                    if switch_class == 'tagInst':
                                        obj._tags.append(Tag(str(child[switch_class]['attributes']['name'])))
                                    continue
                                else:
                                    class_map[switch_class].get_deep(full_data=full_data,
                                                                   working_data=[child],
                                                                   parent=obj,
                                                                   limit_to=limit_to,
                                                                   subtree=subtree,
    ...
    def _instance_get_event(self, session, extension=''):
        ...
        for url in urls:
            ...
            if not session.has_events(url):
                continue
            event = session.get_event(url)
            for class_name in self.__class__._get_switch_classes():
                if class_name in event['imdata'][0]:
                    break
            attributes = event['imdata'][0][class_name]['attributes']
            status = str(attributes['status'])
    ...
    def get_filtered(cls, session, toolkit_class, switch_class, parent=None):
        ...
        for object_data in data:
            name = str(object_data[switch_class]['attributes']['name'])
            obj = toolkit_class(name, parent)
            attribute_data = object_data[switch_class]['attributes']
            obj._populate_from_attributes(attribute_data)
            resp.append(obj)
        return resp
    ...
    def get(cls, session, toolkit_class, switch_class, parent=None):
        ...
        query_url = ('/api/mo/sys%s.json?query-target=subtree&'
                     'target-subtree-class=%s' % (parent_url, switch_class))
        ret = session.get(query_url)
        data = ret.json()['imdata']
        logging.debug('response returned %s', data)
        resp = []
    ...
    def find(self, search_object):
        ...
        :param search_object: NX object to search
        :returns:  List of objects
        """
        result = []
        match = True
...
class BaseNXPhysObject(BaseNXObject):
    ...
    def __init__(self, name='', parent=None, pod=None):
        ...
        if pod:
            ...
            if parent:
                self.pod = parent.pod
        super(BaseNXPhysObject, self).__init__(name=name, parent=parent)
    ...
    def exists(cls, session, phys_obj):
        ...
        switch_nodes = cls.get(session)
        for switch_node in switch_nodes:
            if phys_obj == switch_node:
                return True
        return False
...
class BaseNXPhysModule(BaseNXPhysObject):
    """BaseNXPhysModule: base class for modules  """
    ...
    def __init__(self, slot, parent=None):
        ...
        """
        super(BaseNXPhysModule, self).__init__(name='', parent=None)
    ...
    def get_obj(cls, session, switch_classes, parent_node):
        ...
        if parent_node:
            if not isinstance(parent_node, str):
                ...
                node = parent_node
        pod = '1'
        interface_query_url = ('/api/node/class/' + switch_classes[0] + '.json?'
                                                                     'query-target=self')
        cards = []
        ret = session.get(interface_query_url)
        card_data = ret.json()['imdata']
        for switch_obj in card_data:
            if switch_classes[0] in switch_obj:
                dist_name = str(switch_obj[switch_classes[0]]['attributes']['dn'])
                (slot) = cls._parse_dn(dist_name)
                card = cls(slot)
                card._session = session
                card._populate_from_attributes(switch_obj[switch_classes[0]]['attributes'])
...
class BaseInterface(BaseNXObject):
